...
function authenticate_api {
  ...
  info "Authenticating to the UCP API"
  while [[ -z $AUTH_TOKEN || $AUTH_TOKEN == "null" ]]; do
    debug ">> I am going to sleep for 20 seconds"
    sleep 20s
    info ">> Attempting to get an authentication token"
...
function configure_interlock {
  info "Configuring interlock"
  HTTP_CODE=$(curl -sk --write-out '%%{http_code}' \
  -H "Content-Type: application/json" \
  -H "Accept: application/json" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -X POST "$UCP_URL/api/interlock" \
  -d '{"HTTPPort":8080,"HTTPSPort":8443,"Arch":"x86_64","InterlockEnabled":true}')
  CURL_STATUS=$?
  debug "CURL_STATUS: $CURL_STATUS; HTTP_CODE: $HTTP_CODE"
}
